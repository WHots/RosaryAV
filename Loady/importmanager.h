...
struct ImportUtils 
{
    ...
    /// <param name="apiLocation">Specifies the method of address retrieval: 1 for GetProcAddress, 0 for custom address retrieval 
    /// (using GetProcedureAddressA), and 2 for GetExportAddress. Defaults to 1.</param>
    ...
    T DynamicImporter(const char* method, int apiLocation = 1)
    {
        switch (apiLocation) 
        {
            ...
            case 1:
                return reinterpret_cast<T>(GetProcAddress(hModule, method));
            case 2:
...
private:
    ...
    PVOID __stdcall ImageDirectoryEntryToDataInternal(PVOID Base, BOOLEAN MappedAsImage, ULONG* Size, DWORD SizeOfHeaders, IMAGE_DATA_DIRECTORY* DataDirectory, IMAGE_FILE_HEADER* ImageFileHeader, void* ImageOptionalHeader) const;
    /// <summary>
    ...
    /// <param name="ImageOptionalHeader">Pointer to the image optional header.</param>
    /// <returns>Pointer to the directory entry if found; otherwise, nullptr.</returns>
    PVOID __stdcall ImageDirectoryEntryToData64(PVOID Base, BOOLEAN MappedAsImage, USHORT DirectoryEntry, ULONG* Size, IMAGE_FILE_HEADER* ImageFileHeader, IMAGE_OPTIONAL_HEADER64* ImageOptionalHeader) const;
    /// <summary>
    ...
    /// <returns>Pointer to the directory entry if found; otherwise, nullptr.</returns>
    PVOID __stdcall ImageDirectoryEntryToDataRom(PVOID Base, WORD HeaderMagic, USHORT DirectoryEntry, ULONG* Size, IMAGE_FILE_HEADER* ImageFileHeader, IMAGE_ROM_OPTIONAL_HEADER* ImageRomHeaders) const;
    /// <summary>
    ...
    /// <param name="Size">Pointer to a variable that receives the size of the directory entry.</param>
    /// <returns>Pointer to the directory entry if found; otherwise, nullptr.</returns>
    PVOID __stdcall ImageDirectoryEntryToDataEx(PVOID Base, BOOLEAN MappedAsImage, USHORT DirectoryEntry, ULONG* Size) const;
    /// <summary>
    ...
    IMAGE_SECTION_HEADER* __stdcall ImageRvaToSection(PIMAGE_NT_HEADERS NtHeaders, PVOID Base, ULONG Rva) const;
    /// <summary>
    ...
    PVOID __stdcall ImageRvaToVa(PIMAGE_NT_HEADERS NtHeaders, void* Base, DWORD Rva) const;
    /// <summary>
